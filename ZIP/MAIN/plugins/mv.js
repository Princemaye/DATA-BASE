...
var cineAPI = 'https://cine-download-api.vercel.app';
...
const pk = "`("
const pk2 = ")`"
const channel_url = "https://whatsapp.com/channel/0XeuI3x";
...
async (conn, mek, m, { from, prefix, q, reply, isDev, isMe, isOwners }) => {
    try {
        ...
        if (site === "sublk") {
            ...
            fetchdata = await fetchJson(`${cineAPI}/api/search?query=${encodeURIComponent(text)}&apikey=${apikey}`);
...
async(conn, mek, m, { from, prefix, reply, q, isDev, isMe, isOwners }) => {
    try {
        ...
        if (q.includes(sublkBase)) {
            ...
            fetchdata = await fetchJson(`${cineAPI}/api/movie?url=${q}&apikey=${apikey}`);
            data = fetchdata?.data;
            dlcmd = 'sublk_download';
...
const {
  ...
  cast = []
             ...
             const image =
  (data?.mainImage ? data.mainImage.replace("fit=", "fit").replace(/-\d+x\d+\.jpg$/, '.jpg').replace(/-\d+x\d+\.webp$/, '.webp') : null) ||
  (data?.mainimage ? data.mainimage.replace("fit=", "fit").replace(/-\d+x\d+\.jpg$/, '.jpg').replace(/-\d+x\d+\.webp$/, '.webp') : null) ||
  data?.imageUrls?.[0] ||
  (data?.imageUrl ? data.imageUrl.replace("fit=", "fit").replace(/-\d+x\d+\.jpg$/, '.jpg').replace(/-\d+x\d+\.webp$/, '.webp') : null) ||
  config.LOGO;
...
try{
  ...
  if(q.includes(sublkBase)) {
    ...
    data = fetchdata?.data;
     ...
     if(q.includes('episode')) {
        fetchdata = await fetchJson(`${cineAPI}/api/episode?url=${q}&apikey=${apikey}`);
     } else {
        fetchdata = await fetchJson(`${cineAPI}/api/movie?url=${q}&apikey=${apikey}`);
     }
     data = fetchdata?.data;
...
try {
    ...
    function getDlLinks(json) {
        const urls = json?.data?.downloadUrls;
        if (!Array.isArray(urls)) return null;

        const mainLink = urls.find(v =>
            v?.url?.includes(".online") ||
            v?.url?.includes("CineSubz") ||
            (v?.url?.includes("bot") && !v?.url?.includes("t.me"))
        );

        if (mainLink) return [mainLink.url];
        
        const pixeldrainLinks = urls
    ...
    const sendDoc = async (fileUrl) => {
        try {
            ...
            try {
                messageData.document = (await axios.get(fileUrl, { responseType: "arraybuffer" })).data;
            } catch {
                messageData.document = { url: fileUrl };
            }

            await conn.sendMessage(from, messageData, { quoted: mek });

        } catch (e) {
            await reply(e?.message || "Error download file !");
        }
    };
...
async (conn, mek, m, {
  ...
  try {
    ...
    if (inp.includes(sinhalasubBase)) {
      ...
      response = await fetchJson(`${cineAPI}/api/tvshow?url=${inp}&apikey=${apikey}`);
      mov = response.data;
      cmd = 'cinh_eps_dl';
    } else {
      return reply(`*I do not have a website related to the link you provided. ‼️*`);
    ...
    mov.episodesDetails.forEach(season => {
      ...
      season.episodes.forEach(episode => {
        ...
        const episodeNum = parseInt(episode.number);
...
try {
  ...
  if(q.includes(sinhalasubBase)){
    ...
    fetchdata = await fetchJson(`${cineAPI}/api/episode?url=${inp}&apikey=${apikey}`);
    data = fetchdata?.data;
    dlcmd = 'sublk_download';
  } else {
...
try {
    ...
    const anu = await fetchJson(`${cineAPI}/api/tvshow?url=${inp}&apikey=${apikey}`);
    let mov = anu.data;
    if (!mov) return reply("*Failed to retrieve API information. ❌*");
...
`
    ...
    for (let i = 0; i < data.length; i++) {
        try {
            ...
            const d1 = await fetchJson(`${cineAPI}/api/episode?url=${epUrl}&apikey=${apikey}`);
            ...
            if (d2.includes(cinesubzDownBase)) {
              ...
              if (check?.isUploaded === false) {
                ...
                // Save to your API
                const payload = { url: d2, downloadUrls, uploader: "DarkYasiya" };
