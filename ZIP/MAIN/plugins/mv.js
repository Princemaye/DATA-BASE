...
const apikey = ''; // Not needed if API is public

// Old bases – kept for download logic
...
// Helper to extract download links from Cinesubz API response (used in fallback)
...
async (conn, mek, m, { from, prefix, q, isDev, isMe, isOwners, reply }) => {
    try {
        ...
        const sentMsg = await conn.sendMessage(from, { contextInfo: ctxInfo, image: { url: config.LOGO }, caption: mg }, { quoted: mek });
...
async (conn, mek, m, { from, prefix, q, reply, isDev, isMe, isOwners }) => {
    try {
        ...
        try {
            ...
            console.error(err);
            return reply(`*API request failed for ${site}. Please try another site or later. ⛔️*`);
        }
...
async(conn, mek, m, { from, prefix, reply, q, isDev, isMe, isOwners }) => {
    try {
        ...
        try {
            ...
            console.error(err);
            return reply(`*API request failed. Please check the link or try again later. ⛔️*`);
        }
...
async(conn, mek, m, { from, prefix, q, isDev, isMe, isOwners, reply }) => {
    try {
        ...
        await conn.sendMessage(from, { contextInfo: ctxInfo, image: { url: detImage }, caption: cap }, { quoted: mek });
        await m.react("✔️");
...
async(conn, mek, m, { from, prefix, q, isDev, isMe, isOwners, reply }) => {
    try {
        ...
        for (let i of imageUrls) {
            await conn.sendMessage(jidx, { contextInfo: ctxInfo, image: { url: i }, caption: config.CAPTION || config.FOOTER || '' }, { quoted: mek });
        }
...
cmd({
    ...
    try {
        ...
        // Handle special cases: sinhalasub, moviepluslk, cinesubz
        if (url.includes(sinhalasubBase)) {
            ...
            // Use fallback API (render.com) for Cinesubz downloads
            let MAX_REQ = 1;
            const MAX_RETRY = 3;
            let res;
            ...
            async function fetchRes(){
                return (await axios.get(`https://dark-yasiya-pro-apis.onrender.com/api/download?url=${encodeURIComponent(url)}`)).data;
            }
        ...
        if (Array.isArray(url)) {
            // If we got multiple URLs from fallback, try them in order
            let success = false;
            for (let link of url) {
                try {
                    let finalUrl = link;
        ...
        } else if (url.includes("ddl.sinhalasub.net") || url.includes("sharepoint.com") || url.includes("baiscope") || url.includes("/dl/")) {
...
async(conn, mek, m, { from, prefix, q, isMe, isOwners, reply }) => {
    try {
        ...
        const mass = await conn.sendMessage(from, { contextInfo: ctxInfo, image: { url: "https://raw.githubusercontent.com/DarkYasiyaNew/DARKYASIYA-DATABASE/main/MEDIA/IMAGE/subtitle.png" }, caption: `${mg}\n${config.FOOTER}` }, { quoted: mek });
        await storenumrepdata({ key: mass.key, numrep, method: 'nondecimal' });
...
async (conn, mek, m, { from, prefix, q, isDev, isMe, isOwners, reply }) => {
    try {
        ...
        const msg = await conn.sendMessage(from, { contextInfo: ctxInfo, image: { url: imageUrl }, caption }, { quoted: mek });
...
async(conn, mek, m, { from, prefix, q, isDev, isOwners, isMe, reply }) => {
    try {
        ...
        await conn.sendMessage(from, { contextInfo: ctxInfo, image: { url: epImage }, caption: cap }, { quoted: mek });
        await m.react("✔️");
...
async (conn, mek, m, { from, prefix, q, isMe, isOwners, isDev, reply }) => {
    try {
        ...
        let message = await conn.sendMessage(from, { contextInfo: ctxInfo, text: "*⏩ Starting process...*" }, { quoted: mek });
        ...
        for (let chatId of jid) {
            await conn.sendMessage(chatId, { contextInfo: ctxInfo, image: { url: movImg || config.LOGO }, caption: yt });
        }
        ...
        for (let i = 0; i < data.length; i++) {
            try {
                ...
                if (d2.includes(cinesubzDownBase)) {
                    // Use fallback API for Cinesubz downloads
                    let MAX_REQ = 1;
                    const MAX_RETRY = 3;
                    let res;

                    async function fetchRes(){
                        return (await axios.get(`https://dark-yasiya-pro-apis.onrender.com/api/download?url=${encodeURIComponent(d2)}`)).data;
                    }
                ...
                console.error(err);
                await conn.sendMessage(from, { contextInfo: ctxInfo, text: `*⚠️ Failed to upload Episode ${i+1}*` });
                continue;
            }
        }
